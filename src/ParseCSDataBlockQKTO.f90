...
subroutine ParseCSDataBlockQKTO( i )
    ...
    LOOP_ExcessMixingQKTO: do
        ...
        if (iRegularParamCS(nParamCS,1) == 2) then
            ! Binary mixing terms:
            read (1,*,IOSTAT = INFO) iRegularParamCS(nParamCS,2:5), dRegularParamCS(nParamCS,1:4)
            read (1,*,IOSTAT = INFO) dRegularParamCS(nParamCS,5:6)
        elseif (iRegularParamCS(nParamCS,1) == 3) then
            ! Ternary mixing terms:
            read (1,*,IOSTAT = INFO) iRegularParamCS(nParamCS,2:7), dRegularParamCS(nParamCS,1:3)
            read (1,*,IOSTAT = INFO) dRegularParamCS(nParamCS,4:6)
        else 
