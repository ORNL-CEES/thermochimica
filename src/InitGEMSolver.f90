...
!    lDebugMode              = .FALSE.
    ...
    LOOP_AddPhase: do i = 1, nElements
        ...
        !if ((iPhase(iAssemblageLast(i)) == 0).AND.(nConPhases + nSolnPhases < nElements - nChargedConstraints)) then
        ...
        !elseif ((iPhase(iAssemblageLast(i)) > 0).AND.(nConPhases + nSolnPhases < nElements - nChargedConstraints)) then
            do j = 1,nSolnPhases
                k = nElements - j + 1
