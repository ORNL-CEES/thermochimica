    ...
    !-------------------------------------------------------------------------------------------------------------
    !
    !> \file    ParseCSDataBlockSUBQ.f90
    !> \brief   Parse the data block section corresponding to a SUBQ phase of a ChemSage data-file.
    !> \author  P. Bajpai, M.H.A. Piro
    !> \date    Mar. 25, 2019
    !> \sa      ParseCSDataFile.f90
    !> \sa      ParseCSDataBlock.f90
    !> \sa      ParseCSDataBlockGibbs.f90
    !> \todo    There are a number of lines in SUBQ phases that I do not yet understand.
    ...
    !   ----            ----------      ---------------------
    !   03/04/2018      M.H.A. Piro     Original SUBG code
    !   03/25/2019      P. Bajpai       Modified c ode for SUBQ
    !
    !
    ! Purpose:
    ! ========
    !
    !> \details This module is a modified form of module ParseCSDataBlockSUBG.f90 and the purpose of this subroutine is to parse the
    !! "data block" section of a ChemSage data-file containing a "SUBQ" phase, which represents the two-subblattice
    !! quadruplet approximation of the modified quasichemical model.
    !! This phase differs from many other types of thermodynamic models in that it attempts to capture Short Range Order (SRO)
    ...
    !!
    !! A.D. Pelton, P. Chartrand, G. Eriksson, "The Modified Quasichemical Model IV -- Two-Sublattice Quadruplet Approximation",
    !! Metallurgical and Materials Transactions B, 32A (2001) 1409-1416.
    !!
    !
    !
    ! Pertinent variables:
...
subroutine ParseCSDataBlockSUBQ( i )
    ...
    LOOP_ExcessMixingSUBQ: do
        ...
        if (iRegularParamCS(nParamCS+1,1) == 0) exit LOOP_ExcessMixingSUBQ
...
end subroutine ParseCSDataBlockSUBQ
