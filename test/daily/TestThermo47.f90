    ...
    ! temperature and composition that should correspond to only the zeta and eta phases.
    !
    !-------------------------------------------------------------------------------------------------------------
...
program TestThermo47
    ...
    dPressure               = 1D0
    dElementMass            = 0D0
    dElementMass(94)        = 0.8D0     ! Pu
    dElementMass(92)        = 0.2D0     ! U
    ...
    if (INFOThermo == 0) then
        ...
        if ((DABS(dMolFraction(25) - 0.69563D0)/0.69563D0 < 1D-3).AND. &
        (DABS(dMolFraction(12) - (8.6543D-2))/(8.6543D-2) < 1D-3)) then
            ! The test passed:
