...
program TestThermo52
    ...
    dPressure               = 1D0
    dElementMass            = 0D0
    dElementMass(94)        = 0.2D0
    dElementMass(92)        = 0.8D0
    dElementMass(42)        = 0.001D0
    dElementMass(40)        = 0.0008D0
    ...
    if (INFOThermo == 0) then
        ...
        if ((DABS(dMolFraction(14) - 7.9856D-4)/7.9856D-4 < 1D-3).AND. &
        (DABS(dGibbsEnergySys - (-9.616D4))/(-9.616D4) < 1D-3)) then
            ! The test passed:
