...
program TestParserIO02
    ...
    call ParseCSVFile(DATA_DIRECTORY // 'IO_CSV-02.csv', lPass)
    
    ! Too size of test number check
    if (INFOThermo == 63) then
        ! The test passed:
        print *, 'TestParserIO02: PASS'
        call ThermoDebug
        ! Reset Thermochimica:
        call ResetThermo
        call EXIT(0)
        ...
        ! The test failed.
        print *, 'TestParserIO02: FAIL <---'
        ! Reset Thermochimica:
        call ResetThermo
        call EXIT(1)
    end if

end program TestParserIO02
